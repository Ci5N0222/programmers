...
// 배열의 요소를 정렬하는데 사용
// 배열을 변경하며, 정렬된 배열을 반환
// 일반적으로 문자열로 비교를 수행, 유니코드 코드 포인터 순서대로 정렬하기에 숫자로 이루어진 배열을 정렬할 때,
// 문제가 발생할 수 있다.

function compareFunction(a, b) {
    // a가 b보다 작으면 음수를 반환 (a를 b보다 앞에 위치)
    // a가 b보다 크면 양수를 반환 (a를 b보다 뒤에 위치)
    // a와 b가 같으면 0을 반환
}
// 이 함수는 두개의 요소를 비교하고, 첫 번째 요소 ('a')가 두 번째 요소('b')보다 크면 -1을 반환 두 요소의 위치를 바꾸게 된다.
// 첫 번째 요소가 두 번째 요소보다 작으면 1을 반환하여 요소의 위치를 변경하지 않는다.
// 첫 번째 요소와 두 번째 요소가 같을 경우 'undefined'를 반환하여 요소의 순서를 변경하지 않고 이를 유지